...
namespace DungeonGame
{
    class Map
    {
        public int deep;//lvl depth. deeper=larger maps
        public int height;
        public int width;
        public MapTile[,] tiles;
        public Account.ppos exit;
        public Account.ppos enter;
        int floornum;
        int floorcur;
        int widtun;
        int cavesize;
        int stage;//stage of mapgen. if game stops when map is still generating it will continue from last finished stage saving progress
        ...
        public Map(int d)//map constructor
        {
            if(d>0)
            {
                generatemap(d);
                ...
                for (int j = 1; j < width - 2; j++)
                {
                    if (entdp > 0)
                    {
                        ...
                        if (t == 0 || entdp == 1)
                        {
                            ...
                            {
                                makeexit(tiles[i, j], 0, 1, new List<MapTile>() { tiles[i, j] });
                            }
                            i = height;
                            j = width;
                        }
        ...
        //aftergen. filling map with items-----------------------------------------------------------------------------------------------------------------------------------------------

        public int makeexit(MapTile ent, int range, int count, List<MapTile> list)
        {
            if(range>30+deep)
            {
                Random r = new Random();
                int ra = r.Next(count, floorcur);
                if(ra>=floorcur-1)
                {
                    ent.contains.Add("exit");
                    for (int i = 1; i < height - 2; i++)
                    {
                        for (int j = 1; j < width - 2; j++)
                        {
                            if (tiles[i, j].contains.Contains("exit"))
                            {
                                exit.y = i;
                                exit.x = j;
                            }
                    ...
                    MessageBox.Show(count + "--" + range);
                    return -1;
                }
            }
            for(int i=0;i<4;i++)
            {
                if(ent.neighbours[i].type==1&&!list.Exists(x=>x==ent.neighbours[i]))
                {
                    list.Add(ent.neighbours[i]);
                    count = makeexit(ent.neighbours[i], range + 1, count+1, list);
                }
                if(count<0)
                {
                    break;
        ...
        private int getmaxtunwidth()//max tunnel width depending on map size. max sizes for tun and caves set because higher values cause weird results(>5=long caves, not tunnels)
        {
            double count = 0;
            double d = 12.5;
            int c = height + width;
